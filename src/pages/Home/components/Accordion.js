    ...
    return (
        <div>
            <h2 id="accordion-flush-heading-1">
                <button onClick={() => setShow(!show)} type="button" className="text-lg flex items-center justify-between w-full py-5 font-medium text-left text-green-500 border-b border-green-200 dark:border-green-700 dark:text-green-400" data-accordion-target="#accordion-flush-body-1" aria-expanded="true" aria-controls="accordion-flush-body-1">
                    <span className="text-xl text-green-900 dark:text-white">{question}</span>
            ...
            { show && (   //if show true then show the below three lines else dont 
                <div id="accordion-flush-body-1" className="" aria-labelledby="accordion-flush-heading-1">
                    <div className="py-5 border-b border-green-200 dark:border-green-700">
                        <p className="text-lg mb-2 text-green-500 dark:text-green-400">{answer}</p>
                    </div>
                </div> 
            ) }                           
        </div>
